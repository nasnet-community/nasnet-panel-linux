...
func (u *settingUsecase) SeedDefaults(ctx context.Context) error {
	defaults := []*domain.Setting{
		...
		{Key: "router_degraded_loss_pct", Value: "25", Type: "int", Category: "router",
			...
			Description: "Default probe loss threshold for groups without an override. Display and event only, never reroutes."},
		{Key: "router_degraded_loss_pct_domestic", Value: "", Type: "int", Category: "router",
			Label:       "Degraded loss, domestic (%)",
			Description: "Loss threshold for domestic lines. Blank uses the default loss threshold."},
		{Key: "router_degraded_loss_pct_foreign", Value: "", Type: "int", Category: "router",
			Label:       "Degraded loss, foreign (%)",
			Description: "Loss threshold for secondary lines and VPN tunnels. Blank uses the default loss threshold."},
