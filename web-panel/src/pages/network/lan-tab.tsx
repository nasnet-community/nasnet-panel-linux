                ...
                <CardContent className="space-y-5">
                    {!stored.resolver_ready && (
                        <Alert variant="warning">
                            <TriangleAlert className="h-4 w-4" />
                            <AlertDescription>
                                No dnsmasq service is installed, so the LAN would have no addresses
                                and no name lookups. Install the <code>dnsmasq</code> package on
                                this box first — <code>dnsmasq-base</code> alone is not enough.
                            </AlertDescription>
                        </Alert>
                    )}
                    ...
                    {stored.resolver_ready && !stored.resolver_running && draft.enabled && (
                        <Alert variant="warning">
                            <TriangleAlert className="h-4 w-4" />
                            <AlertDescription>
                                dnsmasq is installed but not running, so devices on the LAN are
                                getting neither addresses nor name lookups. Check{" "}
                                <code>journalctl -u dnsmasq</code> on this box.
                            </AlertDescription>
                        </Alert>
                    )}
                    ...
                    <fieldset disabled={!draft.enabled} className="space-y-4 disabled:opacity-50">
                        <div className="space-y-1.5">
                            ...
                            <p className="text-text-tertiary text-xs">
                                This box's own address on the LAN. Avoid 192.168.1.0/24 — most home
                                ISP routers already use it.
                            </p>
                        </div>
                        ...
                        <p className="text-text-tertiary -mt-2 text-xs">
                            Addresses below the range stay free for devices you address by hand and
                            point a port forward at.
                        </p>
                    ...
                    <div className="border-status-warning/40 bg-status-warning/[0.05] rounded-lg border p-3">
                        <div className="flex items-center justify-between">
                            <div className="pr-4">
                                ...
                                <p className="text-text-secondary mt-0.5 text-xs">
                                    Drops anything arriving on an uplink that isn't the panel, a VPN
                                    inbound or a port forward. The panel is kept open on {domestic}{" "}
                                    only.
                                </p>
                            </div>
                            <Switch
                        ...
                        {draft.input_firewall && !stored.input_firewall && (
                            <p className="text-status-warning mt-2 text-xs">
                                This is the change that can lock you out. You'll be asked to type
                                CONFIRM, then have 90 seconds to keep it.
                            </p>
                        )}
                    </div>
                    ...
                    <div className="flex items-center gap-3">
                        <Button
                            ...
                            disabled={
                                ...
                                save.isPending ||
                                armed ||
                                (draft.enabled && !stored.resolver_ready)
                            }
                        >
