...
const statusConfig: Record<string, { label: string; textClass: string; bgClass: string }> = {
    active: { label: "Active", textClass: "text-emerald-600 dark:text-emerald-400", bgClass: "bg-emerald-500/10 border-emerald-500/20" },
    paused: { label: "Paused", textClass: "text-amber-600 dark:text-amber-400", bgClass: "bg-amber-500/10 border-amber-500/20" },
...
export function StatusBadges({ status }: StatusBadgesProps) {
    ...
    return (
        <motion.span
            ...
            className={cn(
                "inline-flex items-center rounded-full border px-2.5 md:px-3 py-0.5 md:py-1 text-[11px] md:text-xs font-semibold shrink-0",
